...
th.BaseTreeView = function(o) {
	...
	this.appendElement = function() {
		...
		this.calculateNode(dataTree);
		this.showTree(dataTree, 0, 0);
	};
	//
	this.calculateNode = function(node) {
		node.childTreeWidth = 1;
		if(node.child) {
			if(node.child.length>0)
				node.childTreeWidth = 0;
			for(var i=0; i<node.child.length; i++) {
				node.childTreeWidth += this.calculateNode(node.child[i]);
			}
		} 
	...
	this.nodeHeight = 60;
	this.nodeWidth = 150;
	this.showTree =  function(node, lay, prev) {
		...
		if(node.child) {
			var prevW = prev;
			for(var i=0; i<node.child.length; i++) {
				this.showTree(node.child[i], lay+1, prevW);
				prevW += node.child[i].childTreeWidth;
			}

			if(node.child.length>1){
				hr = this.nodeWidth*(node.childTreeWidth - (node.child[0].childTreeWidth + node.child[node.child.length-1].childTreeWidth)/2.0);	

				if(node.childTreeWidth<=1) {
					left = left + hr/2.0;
				} else {
					var t = node.child[0].childTreeWidth;
					left = left + this.nodeWidth* t/2.0+hr/2.0 - this.nodeWidth/2;
				}
			}
		}
		
		this.showNode(node, left, top, hr, hasTop, hasBottom);

	};
	// 解析显示节点
	this.showNode = function(node, left, top, hr, hasTop, hasBottom) {
		...
		var nodeHtml = '<div id="'+node.id+'" class="TreeNode" upNode="'+node.upNode+
						'" style="left:'+left+'px; top:'+top+'px">'+
							...
							'<div class="top-line" style="left: 60px; top:-20px;'+hasTop+'">'+node.childTreeWidth+'</div>'+
							'<div class="bottom-line" style="left: 60px;'+hasBottom+'"></div>'+
							'<div class="hr-line" style="left: '+(this.nodeWidth/2-hr/2.0-15)+'px;top: 38px;'+hasBottom+' width: '+hr+'px;"></div>'+
							'<div class="expendBtn" style="'+hasBottom+'"></div>'+
						'</div>'; //-15
